...
get_best_vol_freq() {
    local power_file=$1
    if [ "$power_file" = "powers.m3.v12" ]; then
...
set_best_power_config_if_needed() {
    ...
    if [ -f $best_vol_freq ]; then
        ...
        local auto_adjust_vol_enabled=`grep ":$power_version:" /etc/config/powers | cut -d ':' -f 7`
        if [ "$auto_adjust_vol_enabled" != "" ]; then
            new_value=$new_value:1:$(get_auto_adjust_vol_params $voltage $freq)
            sed -i "s/.2'/.1'/" /etc/microbt_release
        fi

        sed -i "s/:$power_version:.*/:$power_version:$new_value'/" /etc/config/$default_power_file
...
log_upgrading() {
    ...
    echo "Upgrade firmware from $old_version to $new_version" >> /tmp/miner-state.log
}
...
upgrade_file /bin/upgrade.sh 755 555
