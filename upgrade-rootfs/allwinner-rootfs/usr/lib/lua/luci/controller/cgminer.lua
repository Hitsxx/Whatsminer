...
function action_cgminerrestart()
	luci.util.exec("echo \"`date`|E007|Restart cgminer|Web|Remote restart\" >> /tmp/event-restart-cgminer")
...
function stats()
	...
	for line in stats do
		if ver.modelname == "M1" and ver.hashboardtype == "HB10" then
...
function events()
	...
	local f3 = io.open("/tmp/event-restart-cgminer")
